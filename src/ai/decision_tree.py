...
class HumanData:
    ...
    def __init__(self, id: int, salary: str, gender: str, like_trip: str,
                 buy_wine: str):
        self.id: int = id
...
class DecisionTreeQuestion(object):
    ...
    def __init__(self):
        ...
        self.current_information_entropy = self.calculate_information_entropy(
    ...
    def split_dataset(dataset: List[HumanData], attribute: str):
        ...
        if attribute == "年収":
            split_dataset.append(
                [data for data in dataset if data.is_high_salary])
            split_dataset.append(
                [data for data in dataset if not data.is_high_salary])
        if attribute == "性別":
            split_dataset.append([data for data in dataset if data.is_male])
            split_dataset.append(
                [data for data in dataset if not data.is_male])
        if attribute == "旅行":
            split_dataset.append(
                [data for data in dataset if data.is_like_trip])
            split_dataset.append(
                [data for data in dataset if not data.is_like_trip])
        return split_dataset

    @staticmethod
    def calculate_information_entropy(
            classification_numbers: List[int]) -> float:
        """情報エントロピーを計算する関数"""
        total = sum(classification_numbers)
        return -sum([
            classification / total * math.log2(classification / total)
            for classification in classification_numbers if classification != 0
        ])
    ...
    def calculate_decreasing_information_entropy(self, is_ok: List[int],
                                                 is_no: List[int]):
        information_entropy = sum(is_ok) / sum(
            q.current_classification
        ) * q.calculate_information_entropy(is_ok) + sum(is_no) / sum(
            q.current_classification) * q.calculate_information_entropy(is_no)
        return self.current_information_entropy - information_entropy

    def select_attribute(self, dataset: List[HumanData],
                         except_attributes: List[str]):
        decreasing_information_entropy = {}

        if not ("年収" in except_attributes):
            is_ok_dataset = q.classify_buy_wine(
                [data for data in dataset if data.is_high_salary])
            is_no_dataset = self.classify_buy_wine(
                [data for data in dataset if not data.is_high_salary])
            decreasing_information_entropy[
                "年収"] = self.calculate_decreasing_information_entropy(
                    is_ok_dataset, is_no_dataset)

        if not ("性別" in except_attributes):
            is_ok_dataset = self.classify_buy_wine(
                [data for data in dataset if data.is_male])
            is_no_dataset = self.classify_buy_wine(
                [data for data in dataset if not data.is_male])
            decreasing_information_entropy[
                "性別"] = self.calculate_decreasing_information_entropy(
                    is_ok_dataset, is_no_dataset)
        ...
        if not ("旅行" in except_attributes):
            ...
            is_no_dataset = self.classify_buy_wine(
                [data for data in dataset if not data.is_like_trip])
            decreasing_information_entropy[
                "旅行"] = self.calculate_decreasing_information_entropy(
                    is_ok_dataset, is_no_dataset)

        biggest_attribute = max(decreasing_information_entropy,
                                key=decreasing_information_entropy.get)
        return biggest_attribute, decreasing_information_entropy[
            biggest_attribute]
